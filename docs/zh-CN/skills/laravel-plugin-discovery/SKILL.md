...
description: 通过 LaraPlugins.io MCP 发现并评估 Laravel 包。当用户需要查找插件、检查包的健康状况或评估 Laravel/PHP 兼容性时使用。
origin: ECC
---
...
- 用户想为特定功能查找 Laravel 包（例如"认证"、"权限"、"管理面板"）
- 用户询问"我应该用什么包来……"或"有没有 Laravel 包用于……"
- 用户想检查某个包是否仍在积极维护
...
必须配置 LaraPlugins MCP 服务器。在 `~/.claude.json` 的 mcpServers 中添加：
...
1. 使用 `SearchPluginTool` 并填写相关关键词
2. 应用健康评分、Laravel 版本或 PHP 版本过滤器
3. 查看包含包名、描述和健康指标的结果
...
1. 使用 `GetPluginDetailsTool` 并填写包名
...
1. 将 `laravel_compatibility` 过滤器设置为目标版本后搜索
...
| 场景 | 推荐方式 |
|------|---------|
| "认证用什么包？" | 搜索"auth"并应用健康过滤器 |
| "spatie/package 还在维护吗？" | 获取详情，检查健康评分 |
| "需要 Laravel 12 的包" | 搜索时设置 laravel_compatibility: "12" |
...
1. **始终按健康过滤** — 生产项目使用 `health_score: "Healthy"`
