...
description: 具有钱包或交易权限的自主交易 Agent 的安全模式。涵盖提示注入、消费限额、预发送模拟、熔断器、MEV 保护和密钥处理。
...
自主交易 Agent 面临比普通 LLM 应用更严峻的威胁模型：一次注入或错误的工具调用路径可以直接导致资产损失。

## 何时使用

- 构建能够签署并发送交易的 AI Agent
- 审计交易机器人或链上执行助手
- 为 Agent 设计钱包密钥管理方案
- 为 LLM 提供下单、兑换或国库操作的访问权限

## 工作原理

分层叠加防御措施。没有任何单一检查足够充分。将提示词卫生、消费策略、模拟执行、执行限制和钱包隔离视为独立的控制措施。
...
不要将代币名称、交易对标签、Webhook 或社交动态盲目注入到具有执行能力的提示词中。
...
使用专用热钱包，仅存放当前会话所需的资金。绝不将 Agent 指向主要国库钱包。
...
- 外部数据在进入 LLM 上下文前已完成净化处理
- 消费限额独立于模型输出进行强制执行
- 交易在发送前已完成模拟
- `min_amount_out` 为必填项
- 熔断器在回撤或无效状态时触发暂停
- 密钥来自环境变量或密钥管理器，绝不出现在代码或日志中
- 在适当情况下使用私有内存池或受保护的路由
- 每种策略均已设置滑点和截止时间
- 所有 Agent 决策均记录审计日志，而不仅仅是成功的发送操作
