---
name: csharp-testing
description: 使用 xUnit、FluentAssertions、模拟、集成测试以及测试组织最佳实践的 C# 和 .NET 测试模式。
origin: ECC
---

# C# 测试模式

使用 xUnit、FluentAssertions 和现代测试实践，为 .NET 应用程序提供全面的测试模式。
...
|---|---|
| 测试实现细节 | 测试行为和结果 |
| 共享的可变测试状态 | 每个测试使用全新实例（xUnit 通过构造函数实现此点） |
| 异步测试中使用 `Thread.Sleep` | 使用带超时的 `Task.Delay` 或轮询助手 |
| 对 `ToString()` 输出进行断言 | 对类型化属性进行断言 |
| 每个测试只有一个巨大的断言 | 每个测试一个逻辑断言 |
| 测试名称描述实现 | 以行为命名：`Method_ExpectedResult_WhenCondition` |
