...
description: 临床决策支持系统（CDSS）开发模式。药物相互作用检查、剂量验证、临床评分（NEWS2、qSOFA）、警报严重级别分类，以及与 EMR 工作流的集成。
...
1. **`checkInteractions(newDrug, currentMeds, allergies)`** — 对新药与当前用药和已知过敏史进行检查。返回按严重程度排序的 `InteractionAlert[]`。使用 `DrugInteractionPair` 数据模型。
2. **`validateDose(drug, dose, route, weight, age, renalFunction)`** — 根据体重、年龄调整和肾功能调整规则验证处方剂量。返回 `DoseValidationResult`。
3. **`calculateNEWS2(vitals)`** — 从 `NEWS2Input` 计算国家早期预警评分 2。返回包含总分、风险等级和升级指导的 `NEWS2Result`。
...
相互作用对必须是**双向的**：如果药物 A 与药物 B 存在相互作用，则药物 B 也与药物 A 存在相互作用。
...
| 重大 | 内联警告横幅。橙色。 | 必须确认后才能继续 |
| 轻微 | 内联信息提示。黄色。 | 仅供知晓，无需操作 |

严重警报绝不能自动关闭，也不能实现为浮动通知（toast）。覆盖原因必须存储在审计跟踪中。
...
通过标准：100%。漏掉一个相互作用就是患者安全事件。

### 反模式

- 使 CDSS 检查变为可选或可跳过（未记录原因）
- 将相互作用检查实现为浮动通知（toast）
- 对药物或临床数据使用 `any` 类型
- 硬编码相互作用对，而非使用可维护的数据结构
- 在 CDSS 引擎中静默捕获错误（必须响亮地暴露失败）
