...
description: 医疗应用程序中受保护健康信息（PHI）和个人身份信息（PII）的合规模式。涵盖数据分类、访问控制、审计跟踪、加密和常见泄露途径。
...
保护医疗应用中患者数据、临床医生数据和财务数据的模式。适用于 HIPAA（美国）、DISHA（印度）、GDPR（欧盟）以及一般医疗数据保护。
...
医疗数据保护在三个层面上运作：**分类**（什么是敏感的）、**访问控制**（谁可以查看）和**审计**（谁已查看）。

### 数据分类

**PHI（受保护健康信息）** — 任何能够识别患者身份且与其健康相关的数据：患者姓名、出生日期、地址、电话、电子邮件、国家身份证号码（SSN、Aadhaar、NHS 号码）、病历号、诊断、用药、实验室结果、影像、保险单和理赔详情、预约和住院记录，或上述任何组合。

**PII（非患者敏感数据）** 在医疗系统中：临床医生/员工个人详情、医生费用结构和支付金额、员工工资和银行详情、供应商付款信息。
...
**错误消息：** 切勿在抛给客户端的错误消息中包含患者身份信息。详细信息仅在服务端记录日志。

**控制台输出：** 切勿记录完整的患者对象。使用不透明的内部记录 ID（UUID）——而非病历号、国家身份证号或姓名。

**URL 参数：** 切勿将患者身份信息放入可能出现在日志或浏览器历史记录中的查询字符串或路径段中。仅使用不透明的 UUID。

**浏览器存储：** 切勿将 PHI 存储在 localStorage 或 sessionStorage 中。PHI 仅保存在内存中，按需获取。

**服务角色密钥：** 切勿在客户端代码中使用 service_role 密钥。始终使用匿名/公开密钥，并让 RLS 执行访问控制。

**日志和监控：** 切勿记录完整的患者记录。仅使用不透明的记录 ID（而非病历号）。在发送到错误跟踪服务之前对堆栈跟踪进行脱敏处理。
